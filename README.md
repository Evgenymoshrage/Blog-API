...
- Авторизованные пользователи могут создавать посты и комментарии, редактировать и удалять посты.
...
```text
Схема базы данных инициализируется SQL-миграциями, которые автоматически выполняются Postgres при первом запуске контейнера (migrations/001_init_schema.sql)
```
...
```text
Схема базы данных создаётся с помощью SQL-миграций (migrations/001_init_schema.sql). 
